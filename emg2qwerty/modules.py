...
class SpectrogramNorm(nn.Module):
    ...
    def __init__(self, channels: int, spec_norm: str) -> None:
        super().__init__()
        self.channels = channels

        if spec_norm == 'RollingTimeNorm':
            self.spec_norm = RollingTimeNorm(eps = 0.1) 
        elif spec_norm == 'BatchNorm2d':
            self.spec_norm = nn.BatchNorm2d(channels)
        else:
            raise ValueError(f"Invalid spec_norm, got {spec_norm}")
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(self.spec_norm, RollingTimeNorm):
            x = self.spec_norm(inputs) 
            return x
        
        elif isinstance(self.spec_norm, nn.BatchNorm2d):
